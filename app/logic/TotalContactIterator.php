		...
		foreach ($contacts as $m) {
			if (count($this->contacts) == 0) {
				$c = array(
					...
					'email' => $this->escapeQuotes($m['email']),
					'name' => $this->escapeQuotes($m['name']),
					'lastName' => $this->escapeQuotes($m['lastName']),
		...
		foreach ($contacts as $m) {
			if (count($this->contacts) == 0) {
				...
				if (isset($m['field']) && !empty($m['field'])) {
					...
					$c[$name] = (!empty($m['value']) ? $this->escapeQuotes($m['value']) : null);
				}
				else {
					$c[$name] = null;
				}
...
//				$this->logger->log(print_r($c, true));
				...
				if (isset($m['field']) && !empty($m['field'])) {
					$name = $this->cleanSpaces($m['field']);
					$this->contacts[$i][$name] = (!empty($m['value']) ? $this->escapeQuotes($m['value']) : null);
				}
...
//		$cadena = ereg_replace( "([ ]+)", "", $cadena );
	...
	protected function escapeQuotes($cadena)
	{
		$s = array("'");
		$r = array(' ');
