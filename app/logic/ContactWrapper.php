...
//			$this->logger->log("date: " . print_r($data, true));
	...
	public function convertCompleteContactToJson($contactRow, $customfields, $fieldinstances)
	{
		...
		$object['name'] =  utf8_encode(utf8_decode($contact->name));
		$object['birthDate'] = (!empty($contact->birthDate) ? $this->dateFormat->transformDateFormat($contact->birthDate, 'Y-m-d', 'd/m/Y') : '');
		$object['lastName'] = utf8_encode(utf8_decode($contact->lastName));
		$object['isActive'] = ($contact->status != 0);
		$object['activatedOn'] = (($contact->status != 0)?date('d/m/Y H:i', $contact->status):'');
		$object['isSubscribed'] = ($contact->unsubscribed == 0);
		$object['subscribedOn'] = (($contact->subscribedon != 0)?date('d/m/Y H:i', $contact->subscribedon):'');
			...
			if (isset($fieldinstances[$key])) {
				...
				switch ($field['type']) {
					...
					case 'Number':
						...
						break;
					default:
						$value = utf8_encode(utf8_decode($fvalue['textValue']));
				}
			}
