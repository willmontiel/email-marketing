...
{	
	...
	protected function getDateElement($element)
	{
		...
		if($element->hide) {
			$field = '<input type="text" id="c_' . $element->id . '" name="c_' . $element->id . '" class="form-control date_view_picker" value="' . $element->defaultday . '/' . $element->defaultmonth . '/' . $element->defaultyear . '" data-name="' . $element->name . '">';
		}
	...
	public function getDateValue($element)
	{
		if(isset($this->contact)) {
			if($element->id === 'birthDate') {
				...
				return $datef->transformDateFormat($this->contact->birthDate, 'Y-m-d', 'd/m/Y');
			}
			$fis = Fieldinstance::findFirst(array(
				'conditions' => 'idCustomField = ?1 AND idContact = ?2',
				'bind' => array(1 => $element->id,
