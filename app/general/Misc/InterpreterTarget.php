		...
		if ($this->top && $this->list) {
			switch ($this->criteria) {
				...
				case 'contactlists':
					...
					$this->SQLForContacts = "(SELECT co.* FROM contact co JOIN coxcl cl ON (co.idContact = cl.idContact) WHERE cl.idContactlist IN ({$this->ids}) GROUP BY co.idContact)";
				...
				case 'segments':
					...
					$this->SQLForContacts = "(SELECT co.* FROM contact co JOIN sxc s ON (co.idContact = s.idContact) WHERE s.idSegment IN ({$this->ids}) GROUP BY co.idContact)";
