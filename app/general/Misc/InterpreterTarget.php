		...
		if ($this->top && $this->list) {
			switch ($this->criteria) {
				...
				case 'contactlists':
					...
					$this->SQLForContacts = "(SELECT co.idContact, co.idEmail, co.unsubscribed, DATE_FORMAT(co.birthDate, '%m-%d') FROM contact co JOIN coxcl cl ON (co.idContact = cl.idContact) WHERE cl.idContactlist IN ({$this->ids}) GROUP BY 1, 2, 3, 4)";
				...
				case 'segments':
					...
					$this->SQLForContacts = "(SELECT co.idContact, co.idEmail, co.unsubscribed, DATE_FORMAT(co.birthDate, '%m-%d') FROM contact co JOIN sxc s ON (co.idContact = s.idContact) WHERE s.idSegment IN ({$this->ids}) GROUP BY 1, 2, 3, 4)";
